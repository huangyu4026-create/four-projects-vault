...
新增目录：`red-mansion/05_coordinate_mapping_20260622/`。

本次固定点：正式坐标总库路径不变，查询台仍读同一总库；工作入口从旧 2706 个退役来源段升级为 3754 个新版原子段。GitHub 保存发布验收、双头入口、坐标取材到材料池验收、A/B 复核设计跑、关键脚本、SQLite schema、表行数和本地大库 sha256 清单；SQLite 正式库、词位库、桥表和备份库继续由本地或外部备份负责。

2026-06-23 又新增顶层 `红楼梦人工智能咨询工程/`。它与 `red-mansion/05_coordinate_mapping_20260622/` 使用同一套坐标升级证据，但在 GitHub 上作为独立工程读取：先读 `红楼梦人工智能咨询工程/README.md`，再读 `红楼梦人工智能咨询工程/00_entry_and_route/000C_进入坐标查询_新窗口强制入口_候选正式头.md`。
