...
import formal_honglou_public_engineering_state as public_engineering_state
import formal_honglou_unified_entry_dispatcher as unified_entry_dispatcher


APP_TITLE = "红楼梦研究台"
APP_BUILD = "unified-dispatch-public-state-20260622"
...
COORDINATE_TRIGGER_PHRASE = unified_entry_dispatcher.TRIGGER_COORDINATE
AGGREGATION_TRIGGER_PHRASE = unified_entry_dispatcher.TRIGGER_AGGREGATION
...
def workflow_process_payload(record: dict) -> dict:
    ...
    if isinstance(query_strategy, dict) and query_strategy:
        ...
        strategy_lines = [
            ...
            f"- 查询词策略：{clean_text(query_strategy.get('query_term_strategy') or query_strategy.get('keyword_strategy')) or '未记录'}",
...
def starts_with_unified_query_trigger(question: str) -> bool:
    text = clean_text(question)
    return text.startswith(COORDINATE_TRIGGER_PHRASE) or text.startswith(AGGREGATION_TRIGGER_PHRASE)
...
def honglou_engineering_question(question: str) -> str:
    ...
    if not text:
        return ""
    if starts_with_unified_query_trigger(text):
        return text
    if text.startswith(ENGINEERING_TRIGGER_PHRASE):
        return text
...
def unified_route_context(question: str, task_intent: str = "", requirements: str = "") -> str:
    display_question = strip_honglou_engineering_trigger(question)
    terms = []
    try:
        terms = decomposer.compact_strategy_terms(decomposer.terms_from_question(display_question or question), limit=8)
    except Exception:
        terms = []
    if not terms and clean_text(display_question or question):
        terms = [clean_text(display_question or question)[:40]]
    lines = [
        "Codex查询词策略：页面统一入口初筛；先取原题核心人物、物象、回目、文类、关系词为中心词，归一词和背景锚点只辅助定位，扩展词只补漏，排除同字串题和标题诱导。",
        f"Codex查询词：{'、'.join(terms) if terms else clean_text(display_question or question)}",
        f"Codex查询逻辑策略：{clean_text(task_intent) or '页面统一入口调度'}",
    ]
    req = clean_text(requirements)
    if req:
        lines.append(f"Codex附加要求：{req}")
    return "\n".join(lines)


def dispatch_preflight(question: str, task_intent: str = "", requirements: str = "") -> dict:
    dispatch_question = honglou_engineering_question(question)
    route_context = unified_route_context(dispatch_question, task_intent=task_intent, requirements=requirements)
    packet = unified_entry_dispatcher.dispatch_question(
        question=dispatch_question,
        route_context=route_context,
        out_root=public_engineering_state.OFFICIAL_STATE_ROOT / "入口调度记录",
        execute=False,
    )
    return unified_entry_dispatcher.summarize_packet(packet)


def engineering_state_api() -> dict:
    return public_engineering_state.load_current_state()


def dispatch_api(question: str, route_context: str = "") -> dict:
    question = clean_text(question)
    dispatch_question = honglou_engineering_question(question)
    if not route_context:
        route_context = unified_route_context(dispatch_question)
    packet = unified_entry_dispatcher.dispatch_question(
        question=dispatch_question,
        route_context=route_context,
        out_root=public_engineering_state.OFFICIAL_STATE_ROOT / "入口调度记录",
    ...
    return {
        ...
        "dispatch": unified_entry_dispatcher.summarize_packet(packet),
        "engineering_state": engineering_state_api(),
    }
...
def register_codex_question(
    question: str,
    task_intent: str = "",
    requirements: str = "",
    request_id: str = "",
    entry_dispatch: dict | None = None,
) -> Path:
    CODEX_ANSWER_ROOT.mkdir(parents=True, exist_ok=True)
    ...
    entry_dispatch = entry_dispatch if isinstance(entry_dispatch, dict) else {}
    path = CODEX_PENDING_DIR / f"Q_{request_id}_{question_key}.md"
    if not path.exists():
        dispatch_outputs = entry_dispatch.get("outputs", {}) if isinstance(entry_dispatch.get("outputs"), dict) else {}
        path.write_text(
            "\n".join(
                [
                    "# 红楼梦工程｜Codex 问题入口",
                    ...
                    "## 统一入口调度",
                    "",
                    f"- 调度状态：{entry_dispatch.get('dispatch_status', '未记录')}",
                    f"- 查询头：{entry_dispatch.get('selected_head', '未记录')}",
                    f"- 取材通道：{entry_dispatch.get('lane', '未记录')}",
                    f"- 触发词：{entry_dispatch.get('trigger', '未记录')}",
                    f"- 调度文件：{dispatch_outputs.get('dispatch_json', '')}",
                    "",
...
def talk_api(question: str, top_n: int, task_intent: str = "", requirements: str = "") -> dict:
    ...
    entry_dispatch = dispatch_preflight(display_question or question, task_intent=task_intent, requirements=requirements)
    if not entry_dispatch.get("ok"):
        blocked_reason = clean_text(entry_dispatch.get("blocked_reason")) or clean_text(entry_dispatch.get("reason")) or "统一入口调度阻断。"
        return {
            "package": "",
            "question": question,
            "display_question": display_question,
            ...
            "task_intent": task_intent,
            "requirements": requirements,
            "status": f"入口调度阻断：{blocked_reason}",
            "talk_md": "",
            "article_md": "",
            "top_n": top_n,
            "talk_markdown": "",
            ...
            "answer_checked": False,
            "queue_status": "入口调度阻断",
            "queue_error": blocked_reason,
            "error_category": "entry_dispatch",
            "error_stage": "E00",
            "error_snippet": blocked_reason,
            "return_code": None,
            "error_retryable": True,
            "retry_count": 0,
            "processing": {},
    ...
    if active_item and active_request_id != request_id:
        ...
        active_payload["entry_dispatch"] = entry_dispatch
        active_payload["engineering_state"] = engineering_state_api()
    ...
    pending_path = register_codex_question(
        question,
        task_intent=task_intent,
        requirements=requirements,
        request_id=request_id,
...
def talk_status_api(question_key: str = "", request_id: str = "") -> dict:
    ...
    return {
        ...
        "flow_scorecard": flow_scorecard,
        "auto_corrections": auto_corrections,
        "entry_dispatch": queue_item.get("entry_dispatch") or latest_record.get("entry_dispatch") or {},
        "engineering_state": engineering_state_api(),
    }
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .engineering-state-panel strong {{
      display: block;
      color: var(--ink);
      font-size: 13px;
      margin-bottom: 4px;
    }}
    .engineering-state-panel .state-row {{
      display: flex;
      justify-content: space-between;
      gap: 8px;
    ...
    .engineering-state-panel .state-ok {{ color: #116466; font-weight: 700; }}
    .engineering-state-panel .state-bad {{ color: var(--danger); font-weight: 700; }}
    main {{
      display: grid;
  ...
  <main>
    <aside>
      ...
      <div id="engineering-state-panel" class="engineering-state-panel">
        <strong>工程状态</strong>
        <div>正在读取统一入口与成品态验收。</div>
      </div>
  ...
  <script>
    ...
    function renderEngineeringStatePanel(payload) {{
      const panel = $("engineering-state-panel");
      if (!panel) return;
      const statePayload = payload || {{}};
      const release = statePayload.release_gate?.summary || {{}};
      const eState = statePayload.current_e_state || {{}};
      const ok = statePayload.ok === true && release.ok !== false;
      const pass = release.pass || 0;
      const total = release.total_checks || 0;
      panel.innerHTML = `
        <strong>工程状态 <span class="${{ok ? "state-ok" : "state-bad"}}">${{ok ? "已对齐" : "需检查"}}</span></strong>
        <div>当前：${{esc(eState.stage_id || "E00")}}｜${{esc(eState.stage_name || "统一入口调度")}}</div>
        <div>验收：${{esc(pass)}}/${{esc(total)}} PASS｜答案门：${{eState.answer_allowed ? "可写" : "未开放"}}</div>
        <div class="state-row"><span>入口</span><span>dispatch + release-gate</span></div>
        <div class="meta-kv">${{esc(statePayload.official_state_json || "")}}</div>
      `;
    }}

    async function loadEngineeringState() {{
      try {{
        const payload = await getJSON("/api/engineering-state");
        state.engineeringState = payload;
        renderEngineeringStatePanel(payload);
      }} catch (err) {{
        renderEngineeringStatePanel({{ok: false, current_e_state: {{stage_id: "missing", stage_name: err.message || "读取失败"}}, release_gate: {{summary: {{ok: false, pass: 0, total_checks: 0}}}}}});
      }}
    }}

    function renderEntryDispatch(data) {{
      const dispatch = data.entry_dispatch || {{}};
      if (!dispatch.selected_head && !dispatch.dispatch_status) return "";
      const outputs = dispatch.outputs || {{}};
      const ok = dispatch.dispatch_status === "ready" || dispatch.ok === true;
      return `
        <section class="process-pages">
          <div class="workflow-head">
            <strong>统一入口调度</strong>
            <span>${{ok ? "已通过 E00" : "已阻断"}}</span>
          </div>
          <div class="meta-kv">查询头：${{esc(dispatch.selected_head || "")}}｜通道：${{esc(dispatch.lane || "")}}｜触发词：${{esc(dispatch.trigger || "")}}</div>
          <div class="meta-kv">状态：${{esc(dispatch.dispatch_status || "")}}｜写答案许可：${{dispatch.write_answer_allowed ? "能" : "不能"}}</div>
          ${{outputs.dispatch_json ? `<div class="meta-kv">调度单：${{esc(outputs.dispatch_json)}}</div>` : ""}}
          ${{dispatch.blocked_reason ? `<div class="error">${{esc(dispatch.blocked_reason)}}</div>` : ""}}
        </section>
      `;
    }}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        try:
            if parsed.path == "/":
                ...
                json_response(
                    self,
                    {
                        ...
                        "app_title": APP_TITLE,
                        "app_build": APP_BUILD,
                        "engineering_state_ok": bool(engineering_state.get("ok")),
                        "engineering_state_json": engineering_state.get("official_state_json", ""),
                        "engineering_state_md": engineering_state.get("official_state_md", ""),
                        "release_gate": engineering_state.get("release_gate", {}).get("summary", {}),
                        "codex_bridge": True,
            ...
            elif parsed.path == "/api/engineering-state":
                json_response(self, engineering_state_api())
            elif parsed.path == "/api/dispatch":
                question = qs.get("question", [""])[0].strip()
                route_context = qs.get("route_context", [""])[0].strip()
                json_response(self, dispatch_api(question=question, route_context=route_context))
