...
CORE_FILES = {
    ...
    "interactive_revision_ledger_md": "00AQ_现场交互修正活账.md",
    "interactive_revision_ledger_json": "00AR_现场交互修正活账.json",
...
SEMANTIC_AGGREGATION_TOOL_USAGE_GUIDE = (
    "聚拢查询工程八步法硬锁：先读题、判断题型、拆对象、定主轴、生成入口词令牌；"
    "取词以后进入聚拢法，把候选段落回到语义聚拢中心库、W32聚拢单元、W33聚拢事件和聚拢现场；"
    "聚拢法重点是组织现场、放大/缩小/交集/串域、判材料角色，再送材料池四态裁判；"
    "人物归一、问题树、两两比较、强复合轴、穷尽补点、回原文裁判必须在聚拢工程内部完成。"
)
...
def build_aggregation_eight_step_status(
    *,
    ...
    return {
        ...
        "aggregation_spine": AGGREGATION_EIGHT_STEP_SPINE,
        "aggregation_program_scope": "聚拢查询工程负责语义聚拢取材、聚拢现场组织、材料池入口凭证和答案生成门。",
        "steps": steps,
        "completed_or_ready_steps": completed,
        "total_steps": len(AGGREGATION_EIGHT_STEP_SPINE),
        "answer_gate": {
            "write_answer_allowed": final_ready,
            "program_answer_written": False,
            "reason": "聚拢查询工程负责生成最终答案门；只有 final_prewrite_ready 为 true 时，才允许 Codex 进入写答案。程序本身不越过材料池/原文追证直接写结论。",
        },
    }
...
def render_interactive_revision_ledger_markdown(payload: dict[str, Any]) -> str:
    status = payload.get("revision_status") if isinstance(payload.get("revision_status"), dict) else {}
    ...
    lines = [
        ...
        f"- 结论：{'通过' if status.get('ok') else '阻断'}",
        f"- 是否需要修正账：{status.get('required')}",
        f"- 原因：{status.get('reason', '')}",
        f"- 回到门：{payload.get('return_gate', '')}",
        f"- 当前激活版本：{payload.get('active_versions', '')}",
        f"- 最终综合依据：{payload.get('final_synthesis_basis', '')}",
        "",
        "## 触发与回流",
        "",
        f"- 放行触发：{trigger.get('proceed_signal')}",
        f"- 触发类别：{'、'.join(trigger.get('categories') or [])}",
        f"- 命中词：{'、'.join(trigger.get('matched_terms') or [])}",
        f"- 建议回到门：{trigger.get('recommended_return_gate', '')}",
        "",
        "## 显式字段",
        "",
    ]
    ...
    for key, value in fields.items():
        if str(value or "").strip():
            ...
            wrote = True
    if not wrote:
        lines.append("- 未写显式字段；首轮或由上下文讨论焦点临时判定。")
    lines.extend(["", "## 当前子问题拆解策略快照", ""])
    strategy = payload.get("current_subquestion_decomposition_strategy")
    lines.append(f"- {str(strategy or '').strip() or '未写入 Codex子问题拆解策略。'}")
    subq_status = payload.get("current_subquestion_strategy_status") if isinstance(payload.get("current_subquestion_strategy_status"), dict) else {}
    lines.append(f"- 可查性状态：{'通过' if subq_status.get('ok') else '阻断'}｜{str(subq_status.get('reason') or '').strip()}")
    lines.extend(["", "## 当前子问题快照", ""])
    subqs = payload.get("current_subquestion_snapshot") or []
    if subqs:
        lines.extend(f"- {item}" for item in subqs)
    else:
        lines.append("- 未写入 Codex子问题；不能临时编子问题。")
    lines.extend(["", "## 当前子问题两两交集表快照", ""])
    pairwise_table = payload.get("current_subquestion_pairwise_table_snapshot") or []
    if pairwise_table:
        lines.extend(f"- {item}" for item in pairwise_table)
    else:
        lines.append("- 未写入 Codex子问题两两交集表；可暂以策略队列里的两两可查记录代替。")
    lines.extend(["", "## 当前子问题策略队列快照", ""])
    queue = payload.get("current_subquestion_strategy_queue_snapshot") or []
    if queue:
        lines.extend(f"- {item}" for item in queue)
    else:
        lines.append("- 未写入 Codex子问题策略队列。")
    lines.extend(["", "## 当前取词-查表策略快照", ""])
    strategy_snapshot = payload.get("current_strategy_snapshot") if isinstance(payload.get("current_strategy_snapshot"), dict) else {}
    for key, value in strategy_snapshot.items():
        lines.append(f"- {key}：{str(value or '').strip() or '未写入'}")
...
def build_interactive_revision_ledger_gate(question: str, route_context: str, package_dir: Path) -> dict[str, Any]:
    if query_strategy_gate is None:
        payload = {
            ...
            "revision_status": {
                "ok": False,
                "required": True,
                "reason": "现场修正活账模块未加载，不能证明中途交流已回到正确工程门。",
                "missing": ["缺 formal_honglou_coordinate_material_gate。"],
                "warnings": [],
            },
            "active_versions": "未知",
            "return_gate": "阻断",
            "final_synthesis_basis": "阻断",
        }
    else:
        payload = query_strategy_gate.build_interactive_revision_ledger(question, route_context)
    output_files = {
        "interactive_revision_ledger_md": str(package_dir / CORE_FILES["interactive_revision_ledger_md"]),
        "interactive_revision_ledger_json": str(package_dir / CORE_FILES["interactive_revision_ledger_json"]),
    }
    payload["output_files"] = output_files
    md_path = package_dir / CORE_FILES["interactive_revision_ledger_md"]
    json_path = package_dir / CORE_FILES["interactive_revision_ledger_json"]
    md_path.write_text(render_interactive_revision_ledger_markdown(payload), encoding="utf-8")
    write_json(json_path, payload)
    if not (payload.get("revision_status") or {}).get("ok"):
        raise SystemExit("现场修正活账门阻断：" + str((payload.get("revision_status") or {}).get("reason") or "未知原因"))
    return payload
...
def build_query_logic_strategy_gate(question: str, route_context: str, package_dir: Path) -> dict[str, Any]:
    ...
    subquestion_decomposition_status: dict[str, Any] = {}
    strategy_flexibility: dict[str, Any] = {}
    blocking_reasons: list[str] = []
    gate_available = query_strategy_gate is not None
    if not gate_available:
        blocking_reasons.append("查询逻辑策略门模块未加载，不能证明聚拢查询工程已经执行策略选择。")
    else:
        ...
        subquestion_decomposition_status = query_strategy_gate.subquestion_decomposition_strategy_status(route_context)
        ...
        if requires_subquestions:
            ...
            if not subquestion_decomposition_status.get("ok"):
                blocking_reasons.append(f"子问题拆解策略不合格：{subquestion_decomposition_status.get('reason', '未知原因')}")
    ...
    payload = {
        ...
        "rule": "模板00先用两两交集设计子问题；AI取词之后再进入具体查询逻辑策略和结构转化表，用两两交集查表收点。策略卡是起手卡，允许组合、换卡、偏离，但必须留理由并回原文裁判。",
        "output_files": {
            "interactive_revision_ledger_md": str(package_dir / CORE_FILES["interactive_revision_ledger_md"]),
            "interactive_revision_ledger_json": str(package_dir / CORE_FILES["interactive_revision_ledger_json"]),
    ...
    lines = [
        ...
        f"- 子问题拆解策略状态：{subquestion_decomposition_status.get('reason', '未要求') if subquestion_decomposition_status else '未检查'}",
    ...
    lines.extend(
        [
            ...
            "- 选择模板00时，先用两两交集设计子问题，必须写 Codex子问题，并用 Codex子问题两两交集表或 Codex子问题策略队列证明每条子问题可查。",
...
def build_aggregation_flow_lock(
    ...
    payload = {
        ...
        "aggregation_eight_step_contract": {
            "query_head": "进入聚拢查询",
            "lane": "semantic_aggregation",
            "aggregation_spine": AGGREGATION_EIGHT_STEP_SPINE,
            "aggregation_program_scope": "聚拢查询工程负责语义聚拢取材、聚拢现场组织、材料池入口凭证和答案生成门。",
            "rule": "聚拢查询工程执行完整八步法；不改变归一、收点、交集、路由门、分类、回原文、入材料池、写答案的总顺序。",
        },
        "runtime_bus": {
    ...
    lines.extend(
        [
            ...
            "## 4. 聚拢工程八步主线",
            "",
            f"- 入口令牌：{payload['aggregation_eight_step_contract']['query_head']}",
            f"- 聚拢主线：{' -> '.join(payload['aggregation_eight_step_contract']['aggregation_spine'])}",
            f"- 工程范围：{payload['aggregation_eight_step_contract']['aggregation_program_scope']}",
            f"- 规则：{payload['aggregation_eight_step_contract']['rule']}",
            "",
            "## 5. 唯一新流程必备规则全集",
            "",
        ]
    ...
    for row in review_rows:
        ...
        enriched = {
            **row,
            ...
            "semantic_eight_step_rule": "聚拢查询工程服从完整八步法；取词以后进入聚拢法。",
...
def build_process_inventory(question: str, route_context: str, package_dir: Path) -> dict[str, Any]:
    ...
    payload = {
        ...
        "stages": catalog,
        "hard_gates": [
            "现场交互修正活账失败：中途交流后不能证明回到子问题策略讨论节点或取词-查表策略讨论节点，工程停在修正账门。",
        ...
        "current_true_flow": [
            "现场交互修正活账：首轮建立版本线；中途交流后依据讨论焦点判定回到子问题策略讨论节点或取词-查表策略讨论节点，再决定重跑范围。",
...
def build_codex_pipeline_audit(
    ...
    profile = decomposer.route_context_profile(route_context)
    codex_terms = profile.get("codex_terms", []) if isinstance(profile, dict) else []
    query_term_strategy = profile.get("query_term_strategy", "") if isinstance(profile, dict) else ""
    ...
    checks = [
        ...
        },
        {
            ...
            "expected": "Codex 根据题意和工程能力生成查询词策略、查询词、强复合、优先库和查证步骤；没有查询词策略则停在入口词门。",
            "status": _audit_status(bool(codex_terms) and bool(clean(query_term_strategy))),
            "evidence": f"Codex查询词策略：{clean(query_term_strategy) or '缺失'}；Codex查询词：{'、'.join(codex_terms) or '缺失'}；强复合组：{len(compound_groups)}；优先库：{'、'.join(preferred_libraries) or '缺失'}。",
            "next_fix": "" if codex_terms and clean(query_term_strategy) else "保持入口词门：补 Codex查询词策略 和 Codex查询词后再运行证据池。",
        },
        {
            "stage": "3. 工程执行查库查原文",
...
def build_experience_codex_protocol(
    ...
    strategy_layer = {
        "route_center": judgment_payload.get("route_center", ""),
        "query_term_strategy": profile.get("query_term_strategy", ""),
        "codex_terms": profile.get("codex_terms", []),
        "codex_libraries": profile.get("codex_libraries", []),
        "compound_groups": profile.get("compound_groups", []),
        "preferred_axes": judgment_payload.get("preferred_axes", []),
        "rule": "Codex 读题型经验后先生成查询词策略，再生成查询词、库线和强复合组；本地工程只执行这些策略。",
    }
    correction_layer = [
    ...
    lines.extend(
        [
            ...
            f"- Codex查询词策略：{strategy_layer['query_term_strategy'] or '未提供'}",
...
def build_closed_loop(
    ...
    interactive_revision_ledger = build_interactive_revision_ledger_gate(question, route_context, package_dir)
    ...
    steps.append(
        step(
            "现场交互修正活账",
            bool((interactive_revision_ledger.get("revision_status") or {}).get("ok")),
            {
                "required": (interactive_revision_ledger.get("revision_status") or {}).get("required"),
                "return_gate": interactive_revision_ledger.get("return_gate", ""),
                "active_versions": interactive_revision_ledger.get("active_versions", ""),
                "interactive_revision_ledger_md": interactive_revision_ledger.get("output_files", {}).get("interactive_revision_ledger_md", ""),
                "interactive_revision_ledger_json": interactive_revision_ledger.get("output_files", {}).get("interactive_revision_ledger_json", ""),
            },
        )
    )
    steps.append(
    ...
    steps.append(
        step(
            ...
            True,
            {
                ...
                "aggregation_spine": eight_step_status.get("aggregation_spine", []),
                "aggregation_program_scope": eight_step_status.get("aggregation_program_scope", ""),
    ...
    manifest = {
        ...
        "smoke": smoke_result,
        "results": {
            "interactive_revision_ledger": interactive_revision_ledger,
