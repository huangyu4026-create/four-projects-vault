...
def build_coordinate_eight_step_status(
    ...
    steps = [
        _eight_step_row(
            1,
            "归一",
            "完成" if has_query_terms and has_strategy else "阻断",
            f"Codex查询词={'有' if has_query_terms else '缺'}；查询逻辑策略={'有' if has_strategy else '缺'}；取材门=坐标。",
            "坐标入口词路 + 人物变量归一",
        ...
        _eight_step_row(
            ...
            "路由门",
            "完成",
            f"material_admission_status={admission_status}；第一取材入口=坐标材料门；禁止先进入聚拢查询=true。",
            "坐标材料门准入凭证",
        ...
        _eight_step_row(
            ...
            "写答案",
            "未执行",
            "坐标材料包程序只到材料池入口；最终答案必须等待材料池四态、精读材料词、写作前原文追证完成后由后身生成。",
            "硬阻断：候选不作答",
    ...
    return {
        ...
        "same_spine_as_aggregation_head": True,
        ...
        "answer_gate": {
            ...
            "reason": "坐标头只负责编码坐标取材、库表验链、送入材料池；不能把候选升级成最终答案。",
        },
    }
...
def coordinate_admission_voucher(bundle: dict[str, Any], route_context: str, paths: dict[str, str]) -> dict[str, Any]:
    ...
    admission_status = "准入待判" if rows and has_source_anchor and has_route_note and has_strategy_note else "阻断待补"
    admission_reason = (
        "具备坐标取材来源、原子段/回目/原文句等锚点，并具备 Codex 查询词路和查询逻辑策略；可交给材料池四态判定。"
        if admission_status == "准入待判"
        else "缺少候选、来源锚点、Codex 查询词路或查询逻辑策略，只能作为补证对象，不能进入写作。"
    )
...
def render_admission_voucher_markdown(voucher: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## 双头八步主线状态",
        "",
        f"- 查询头：{clean(eight_step_status.get('query_head'))}",
        f"- 共用主线：{' -> '.join(eight_step_status.get('shared_spine') or [])}",
        f"- 工具差异限定：{'是' if eight_step_status.get('tool_difference_only') else '否'}",
...
def build_package(question: str, out_root: Path, limit: int, route_context: str = "") -> dict[str, str]:
    ...
    if not strategy_value:
        raise SystemExit("坐标材料门缺 Codex查询逻辑策略：AI取词后必须先进入 000E_查询逻辑策略模板组_新窗口学习入口.md 选择一个策略模板。")
